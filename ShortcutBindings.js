...
function install(owner, modifier) {
    return "local s=_windowpeek_shortcuts_v1; if not s then s={binds={}}; _windowpeek_shortcuts_v1=s; "
        + "function s.stop() s.owner=nil; for _,b in ipairs(s.binds) do b:set_enabled(false) end; s.timer:set_enabled(false) end; "
        + "s.timer=hl.timer(function() s.stop() end,{timeout=750,type='repeat'}); end; "
        + "for _,b in ipairs(s.binds) do b:set_enabled(false) end; "
        ...
        + "if not s.byModifier[modifier] then local binds={}; local keypad={90,87,88,89,83,84,85,79,80,81}; "
        + "for digit=0,9 do local top=modifier:find('SHIFT',1,true) and ('code:'..(digit==0 and 19 or 9+digit)) or tostring(digit); "
        + "for _,key in ipairs({top,'code:'..keypad[digit+1]}) do "
        + "table.insert(binds,hl.bind(modifier..' + '..key,function() if not s.owner then return {ok=false} end; "
        + "hl.dispatch(hl.dsp.event('windowpeek-shortcut-digit,'..s.owner..','..digit)); "
        + "end,{auto_consuming=true,description='WindowPeek: visible window '..digit})) end end; s.byModifier[modifier]=binds end; "
        + "s.binds=s.byModifier[modifier]; s.owner=" + token(owner) + "; "
        + "for _,b in ipairs(s.binds) do b:set_enabled(true) end; s.timer:set_timeout(750); ";
}
function renew(owner) {
    return "local s=_windowpeek_shortcuts_v1; if s and (not s.owner or s.owner==" + token(owner)
        ...
        + "; for _,b in ipairs(s.binds) do b:set_enabled(true) end end; s.timer:set_timeout(750) end; ";
}
